...
- **自动更新**：内置更新器直接读 GitHub Releases，能自动检查 / 自动安装，也能手动点。
  安装是**原地替换** App 本体，桌面布局和素材不受影响。
...
- **自动检查更新**（默认开）：启动后查一次，之后每天一次。
- **发现新版本后自动下载并安装**（默认关 —— 它会重启 App，这件事交给你决定）。
- **检查更新** 按钮：随时手动查一次。
- 菜单栏面板的「设置」按钮上会出现一个橙色小点，表示有新版可用。

更新包**先校验再安装**：SHA-256 一定查；在 `Info.plist` 里配上 Ed25519 公钥之后还会验签，
而且**缺签名文件直接拒绝，不会悄悄降级成只查哈希**。

安装时 App 会在原地被替换，`~/Library/Application Support/MemeDesk/` 里的桌面布局、素材路径和
设置都在 App 包之外，所以不受影响。装完自动重启，桌面原样恢复。

**关于联网**：检查更新是全项目唯一会联网的地方，只访问 `api.github.com` 和 GitHub 的发布文件，
不带任何标识信息。把「自动检查更新」关掉之后，就只有你手动点按钮时才会发请求。
...
Finder、Dock 和欢迎页看到的永远是同一张脸。

**菜单栏按钮是例外，而且是刻意的。** 那里不放 App 图标（一个橙色圆角方块，既比系统图标"重"、
又比它们小一圈），而是**模板图**：颜色交给系统决定 —— 浅色菜单栏是黑的，深色是白的。
在设置里随时切换，改完立刻生效。图案分两组：
...
- 发布包**只含 arm64**（Apple 芯片）。0.1.7 短暂发过双架构，但下载体积要翻倍
  （zip 0.76 MB → 1.34 MB），从 0.1.8 起改回只发 arm64 —— **Intel Mac 请从源码构建**，
  构建步骤见下，源码是跨架构的
